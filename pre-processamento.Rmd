...
- Aprendizado Não Supervisionado de Clusterização
...
Importando a base de dados para o ambiente do R:
...
nulos <- as.data.frame(colSums(is.na(df)))
rmarkdown::paged_table(nulos)
...
str(df)
```
A única coluna que necessita de tratamento é a coluna date, que será transformada para formato de data.

Nova classe da coluna date:
```{r Formato datetime}
...
rmarkdown::paged_table(head(df_id_duplicados))
```
...
Verificando a existência de duplicidades após tratamento nas observações desatualizadas:
...
corrplot(sqft_corr_matrix, method = "color",tl.col = "black")
```
```{r Visualizando valores}
matrix <- as.data.frame(sqft_corr_matrix)
rmarkdown::paged_table(head(matrix))
```
...
ggplot(df, aes(x = basement_percent*100)) +
  geom_histogram(binwidth = 5, fill = "#4979A9", color = "white", alpha = 0.7) +
...
- sqft_living: Tamanho de área hábitavel(construida) em pés;
...
grafico1 <- ggplot(df, aes(x = sqft_living, y = price)) +
  geom_point(color = "#4979A9", alpha = 0.3) +
  labs(title = "Tamamho do imóvel x preço",
       x = "Tamanho do imóvel",
       y = "Preço") +
  theme_minimal()
...
grafico2<- ggplot(df, aes(x = sqft_living, y = price_sqft)) +
  geom_point(color = "#4979A9", alpha = 0.3) +
  labs(title = "Tamamho do imóvel x preço por feet quadrado",
       x = "Tamanho do imovel",
...
```{r}
# Paleta
paleta2 <- c("#FFFFCC", "#FED976", "#FEB24C", "#FD8D3C", "#FC4E2A", "#E31A1C", "#BD0026")

# Ajuste a escala de cores conforme necessário
cor_escala <- colorNumeric(palette = paleta2 , domain = df$price_sqft)
...
map_leaflet <- map_leaflet %>%
  addCircleMarkers(lng = ~long, lat = ~lat,
                   radius = 2,  # Ajuste o tamanho dos marcadores conforme necessário
...
# Exibindo 
rmarkdown::paged_table(head(df_zipcode))
```
...
# Visualizando
rmarkdown::paged_table(head(group_sqft_price_mean))
```
...
df$cor <- ifelse(df$grupo == "A", "#E41A1C",
                 ifelse(df$grupo == "B", "#377EB8",
                        ifelse(df$grupo == "C", "#4DAF4A", "#984EA3")))
...
Os tratamentos de valores outliers serão realizados nas colunas bedrooms e price.

Primeiramente, iremos verificar a preditora bedrooms:

```{r}
boxplot_func <- function(y_variable) {
  ggplot(df, aes(x = "", y = {{y_variable}})) +
    geom_boxplot(fill = "red", alpha = 0.5) +
    labs(title = paste("Boxplot -", deparse(substitute(y_variable))),
         x = "",
         y = "Valor") +
    theme_minimal()
}
...
# Visualizando 
boxplot_func(bedrooms)
```
Ánalisando o boxplot gerado, temos um imóvel com mais de 30 quartos dentro da base de dados. Esta observação será retirada da analise para não influenciar na modelagem.

```{r}
# Retirando observação com mais de 30 quartos
df <- df[df$bedrooms < 30, ]
```

Observando os valores outliers de preço.
```{r}
boxplot_func(price)
```
De acordo com o boxplot gerado acima, iremos definir um corte para trabalhar apenas com apartamentos menores que 4 milhões.
```{r}
# Definindo corte
df <- df[df$price < 4000000, ]
```

### Tratamento da Preditora Quartos

Conforme analise exploratória, a relação entre o preço e o número de quartos não segue um padrão de linearidade, por conta disso, iremos criar uma categoria para esta coluna.

```{r}
# Transformando em categoria
df$categoria_quartos <- ifelse(df$bedrooms > 5, ">5", as.character(df$bedrooms))
...
rmarkdown::paged_table(head(df))
```
O dicionário de dados para o dataframe processado:

- price: Preço de venda;
- categoria_quartos: número de quartos em categoria;
