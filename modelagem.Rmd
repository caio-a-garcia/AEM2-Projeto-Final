...
[Modelagem]{style="color: #436F9B;font-size:36px;"}
...
rmarkdown::paged_table(head(df_results))
```
...
rmarkdown::paged_table(df_results)
```
...
# Salvando hiperparametros otimizados em um arquivo CSV
#write_csv(rf_best_hip, "data/rf_best_hip.csv")

#rf_best_hip
```

**OBS :Como o tempo de processamento é elevado para o modelo de Floresta Aleatória, os hiperparamentros otimizados serão salvos em um arquivo csv para não ser preciso rodar todas as interações novamente**

```{r}
# Lendo arquivo CSV com hiperparametros otimizados
rf_best_hip <- read.csv("data/rf_best_hip.csv")
...
## Comparando modelos com e sem clusterização

Com o objetivo de avaliar a contribuição da análise não supervisionada de agrupamento realizada na etapa de pré-processamento, foram aplicados os modelos de regressão no dataset que mantém as colunas de latitude, longitude e zipcode. Abaixo, uma visualização do dataframe sem clusterização:
...
Para esse comparativo, foram otimizados novamente os hiperparâmetros da floresta aleatória e do boosting, de modo a compreendermos o potencial da modelagem sem a análise não supervisionada. Os resultados podem ser observados abaixo:

```{r}
# Lendo arquivo csv
df_semcluster_results <- read.csv("data/df_semcluster_results.csv")

# Visualização
rmarkdown::paged_table(df_semcluster_results)
```

Criando uma tabela de comparação percentual no ganho de predição ao não utilizar a clusterização:

```{r}
# Criando função do calculo percentual de EQM e MAPE
comp_calc <- function(modelo, metrica) {
  resultado <- ((df_results[df_results$Modelo == modelo, metrica] - df_semcluster_results[df_semcluster_results$Modelo == modelo, metrica])/df_results[df_results$Modelo == modelo, metrica])*100
  return(resultado)
}
...
df_cluster_compare <- data.frame(
    Modelo = c("Linear", "Ridge", "Floresta Aleatória", "Gradient Boosted"),
    EQM_percent = c(comp_calc("Linear", "EQM"), comp_calc("Ridge", "EQM"), comp_calc('Floresta Aleatória', "EQM"), comp_calc("Gradient Boosted", "EQM")),
    MAPE_percent = c(comp_calc("Linear", "MAPE"), comp_calc("Ridge", "MAPE"), comp_calc('Floresta Aleatória', "MAPE"), comp_calc("Gradient Boosted", "MAPE"))
)
  
# Visualizando os resultados
rmarkdown::paged_table(df_cluster_compare)

```

Observa-se uma melhoria percentual no erro para todos os modelos analisados. No caso do modelo de gradient boosting, em particular, verificou-se uma redução de 32% no erro quadrático médio e de 22% no erro percentual absoluto. Esses resultados indicam que a ausência da clusterização está associada a um ganho significativo no poder preditivo.
